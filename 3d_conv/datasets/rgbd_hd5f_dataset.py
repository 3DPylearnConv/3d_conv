...
class HDF5_Iterator():
    ...
    def next(self):
        ...
        if not 'max_grasp_type' in self.dataset.h5py_dataset.keys():
            self.dataset.h5py_dataset.create_dataset('max_grasp_type', (1,))
            self.dataset.h5py_dataset['max_grasp_type'] = self.dataset.h5py_dataset['grasp_type'][:].max()
        ...
        for i in range(len(finger_indices)):
            ...
            grasp_type = rgbd_label_selection[i]
